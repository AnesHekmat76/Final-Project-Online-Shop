...
const CartItem: React.FC<{ product: cartItem; number: number }> = ({ product, number }) => {
  ...
  const minusButtonClick = () => {
    dispatch(cartAction.removeItem(product.id));
  };
  const trashButtonClick = () => {
    dispatch(cartAction.deleteItems(product.id));
  };
  return (
    <div
      ...
      <p className="w-3/12 lg:w-2/12 title-limit-1 lg:px-3">{product.title}</p>
      <p className="hidden w-3/12 lg:block pl-6 description-limit-2">{product.description}</p>
      ...
      <p className="lg:text-base w-12/100 text-center">{product.price} $</p>
